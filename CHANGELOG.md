...
Open `/plugins` inside a Codex session, add `umyunsang/ThyQuery` as a marketplace, and install **ThyQuery for Codex** from the list. `codex plugin marketplace add` and `codex plugin add` do the same thing from a shell.
